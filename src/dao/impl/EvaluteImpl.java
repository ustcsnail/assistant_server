...
public class EvaluteImpl implements IResponseTrade {
	private Connection conn= null;
	private PreparedStatement pstmt = null;
	public  EvaluteImpl(Connection conn){
		this.conn=conn;
	}
	@Override
	public boolean doCreate(ResponseTrade bbs) throws Exception {
		...
		boolean flag = false;
		String sql ="insert into evalute_res(author,title,trade_id,time,id) values(?,?,?,?,?)";
	...
	public List<ResponseTrade> findAll(int id) throws Exception {
		...
		String sql = "select * from evalute_res  where trade_id=? order by time DESC";
	...
	public List<ResponseTrade> findById(int id) throws Exception {
		String sql ="select * from evalute_res where id = ?";
		List<ResponseTrade> all = new ArrayList<ResponseTrade>();
		this.pstmt=this.conn.prepareStatement(sql);
		this.pstmt.setInt(1, id);
		ResultSet rs = this.pstmt.executeQuery();
