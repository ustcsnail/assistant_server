   ...
   public TradeServlet() {
	...
	public String bbsSelect(HttpServletRequest req,HttpServletResponse resp) throws IOException{
		...
		case 1:
			...
			try {
				all = DAOFactory.getITradeDAPInstance().findAll(name);
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
		...
		case 3:
			...
			try {
				...
				all = DAOFactory.getITradeDAPInstance().findById(bbs_id);
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
		...
		Iterator<Trade> iterator=all.iterator();
		...
		for(Trade bbs:all){
			...
			try {
				...
				jsonObj[i].put("title",bbs.getTitle() );//向对象里面添加值
		         ...
		         jsonObj[i].put("responseBBS",bbs.getReponsetrade());
	...
	public String bbsInsert(HttpServletResponse reps) throws Exception{
		...
		try{
		if(DAOFactory.getITradeDAPInstance().doCreate(bbs))
		  {
	...
	public Trade parseJson(String jsonString) throws JSONException, UnsupportedEncodingException{
		Trade bbs = new Trade();
